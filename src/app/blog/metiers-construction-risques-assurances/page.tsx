...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title,
children }) => (
...
const RiskCard: React.FC<{ title: string; risk: string; insurance: string; }> = ({ title,
risk, insurance }) => (
    <div className="bg-secondary/20 p-6 rounded-xl border border-border/50">
        ...
        <p className="mt-2"><strong>Assurance Clé :</strong> <Highlight
text={insurance} words={['Assurance']} /></p>
    </div>
);
...
Équipements et Outils :</strong> Vos outils sont votre gagne-pain. Cette <Highlight
text="assurance" words={['assurance']} /> les protège contre le vol et le bris, que ce
...
contamination accidentelle des sols ou de l'eau, un risque présent même dans des métiers où on l'attend le moins.</li>
                        ...
                        <div className="!mt-12 border-t border-border/50 pt-8">
                            <p className="font-semibold text-primary">
                                Ne considérez pas votre <Highlight text="assurance"
words={['assurance']} /> comme une simple dépense, mais comme un
investissement stratégique dans la pérennité de votre entreprise. Chaque métier a
ses risques ; nous avons les solutions.
                            </p>
                        </div>
                    </div>
                </div>
